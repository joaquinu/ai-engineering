...
digraph RAG_Workflow {
    ...
    // Cluster 1: Force horizontal alignment with rank=same
    subgraph cluster_retrieve {
        label = "Retrieve";
        { rank=same; "Retrieve"; "Embed"; "Search"; "Chunks"; }
        "Retrieve" -> "Embed" -> "Search" -> "Chunks";
    }

    // Cluster 2: Force horizontal alignment
    subgraph cluster_augment {
        label = "Augment";
        { rank=same; "Augment"; "Format"; "Combine"; }
        "Chunks" -> "Augment" -> "Format" -> "Combine";
    }

    // Cluster 3: Force horizontal alignment
    subgraph cluster_generate {
        label = "Generate";
        { rank=same; "Generate"; "LLM"; "Cite"; }
        "Combine" -> "Generate" -> "LLM" -> "Cite";
    }
}
```

Every RAG system follows this pattern. The differences between production RAG systems are in the details of each step: how you chunk, how you embed, how you search, and how you construct the prompt.
...
Documents are too long to embed as single vectors. Instead, you split documents into chunks and embed each chunk separately.
...
The indexing phase runs once per document (or when documents update). The querying phase runs on every user request.
